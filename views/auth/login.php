...
// Ganti dengan Client ID Google milik kamu
...
<body class="bg-gradient-to-br from-emerald-950 via-slate-900 to-emerald-900 min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
    ...
    <div class="relative z-10 w-full max-w-md bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl border border-white/20 p-8">
        ...
        <div class="text-center mb-6">
            ...
            <p class="text-slate-500 text-xs mt-1">Masuk menggunakan email Anda</p>
        </div>
        ...
        <!-- LOGIN DENGAN EMAIL -->
        <form action="../../app/controllers/AuthController.php?action=send_otp" method="POST" class="space-y-4">
            <!-- EMAIL -->
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="email" id="email" name="email" required autocomplete="email" placeholder="Masukkan email Anda" class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all duration-200 text-slate-800">
                </div>
            </div>

            <!-- TOMBOL OTP -->
            <button type="submit" class="w-full mt-2 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-bold py-3 rounded-xl shadow-lg shadow-emerald-600/30 hover:shadow-emerald-600/40 transition-all duration-200 flex items-center justify-center gap-2 text-sm">
        ...
        <div class="mt-3 text-center">
            <p class="text-[11px] text-slate-400">Kode OTP akan dikirim ke email Anda dan berlaku selama 5 menit.</p>
        </div>
        ...
        <!-- GOOGLE LOGIN -->
        <button type="button" onclick="triggerGoogleLogin()" class="w-full bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-semibold py-2.5 px-4 rounded-xl shadow-sm hover:shadow transition-all duration-200 flex items-center justify-center gap-3 text-sm">
        ...
        <!-- FOOTER -->
        <div class="mt-6 pt-4 border-t border-slate-100 text-center text-xs text-slate-500 space-y-2">
            <div>
                Belum punya akun?
                <a href="register.php" class="font-bold text-emerald-600 hover:text-emerald-700 transition-colors ml-1">Daftar di sini</a>
            </div>
            <div>
                <a href="../../index.php" class="inline-flex items-center gap-1.5 font-semibold text-slate-500 hover:text-emerald-600 transition-colors">
    ...
    <script>
        window.onload = function () {
            if (typeof google !== 'undefined') {
                google.accounts.id.initialize({
                    client_id: "<?php echo htmlspecialchars($googleClientID); ?>",
        ...
        function triggerGoogleLogin() {
            if (typeof google === 'undefined') {
                alert('Gagal memuat Google Login. Periksa koneksi internet Anda.');
                return;
            }

            google.accounts.id.prompt(function (notification) {
                if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
                    alert('Login Google tidak dapat dibuka. Pastikan Google Client ID sudah benar.');
                }
            });
        }
