...
// Normalisasi Penanganan Session User (sama seperti index.php)
...
// Halaman ini butuh login (kartu rewards melekat ke akun pelanggan)
if (!$isLoggedIn) {
...
<body class="bg-slate-50 text-slate-800 min-h-screen">
    ...
    <!-- Header (disederhanakan dari index.php) -->
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16 gap-4">
                ...
                <div class="flex items-center gap-3">
                    <a href="views/profile/index.php" class="text-xs sm:text-sm font-semibold text-slate-500 hover:text-emerald-600 flex items-center gap-1.5">
    ...
    <main class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">Kumpulkan poin dari setiap belanja baju dan tukarkan dengan hadiah menarik.</p>
        </div>
        ...
        <section class="card-shell rounded-3xl p-6 sm:p-8 text-white shadow-2xl relative overflow-hidden">
            ...
            <div class="relative z-10 flex items-start justify-between mb-6">
                ...
                <button onclick="toggleInfoTooltip()" class="w-7 h-7 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-xs">
                    <i class="fa-solid fa-info"></i>
                </button>
            </div>

            <div id="infoTooltip" class="hidden relative z-10 -mt-3 mb-4 bg-white/10 border border-white/20 rounded-xl p-3 text-xs text-slate-100">
                Dapatkan 1 poin setiap belanja Rp500.000. Kumpulkan 10 poin untuk klaim hadiah menarik! Sisa nominal di bawah Rp500.000 tidak diakumulasi ke transaksi berikutnya.
            </div>
            ...
            <div class="relative z-10 mt-4 space-y-2">
                ...
                <div class="w-full h-2.5 bg-white/10 rounded-full overflow-hidden">
                    <div id="progressBar" class="h-full bg-gradient-to-r from-emerald-400 to-yellow-400 transition-all duration-500" style="width:0%"></div>
                </div>
            </div>

            <div class="relative z-10 mt-5 bg-white/10 border border-white/15 rounded-2xl p-4 flex items-center gap-3">
                <div class="w-11 h-11 rounded-xl bg-yellow-400/20 flex items-center justify-center text-yellow-300 text-lg shrink-0">
        ...
        <!-- Demo helper: no payment gateway is wired into this codebase yet,
             so this simulates ON payment_success to exercise the same
             /api/rewards.php?action=calculate path a real order would call. -->
        <section class="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
            <div class="text-xs text-amber-800">
                <i class="fa-solid fa-flask mr-1"></i>
                <b>Mode Demo:</b> belum ada gateway pembayaran di codebase ini — gunakan tombol ini untuk mensimulasikan transaksi fashion yang berhasil dibayar.
            </div>
            <div class="flex items-center gap-2 shrink-0">
                ...
                <button onclick="simulatePurchase()" class="bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold px-3 py-2 rounded-lg whitespace-nowrap">
        ...
        <section class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="flex gap-2 p-3 border-b border-slate-100 overflow-x-auto">
                <button class="tab-btn active px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all" data-tab="active" onclick="switchTab('active')">Kartu Aktif</button>
                <button class="tab-btn px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all text-slate-600 hover:bg-slate-100" data-tab="points" onclick="switchTab('points')">Riwayat Poin</button>
                <button class="tab-btn px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all text-slate-600 hover:bg-slate-100" data-tab="rewards" onclick="switchTab('rewards')">Riwayat Rewards</button>
            </div>
            ...
            <div class="p-5">
                ...
                <div id="tabPoints" class="hidden overflow-x-auto">
                    <table class="w-full text-xs sm:text-sm">
                        <thead>
                            <tr class="text-left text-slate-400 uppercase text-[10px] tracking-wider border-b border-slate-100">
                        ...
                        <tbody id="pointsHistoryBody">
                            <tr><td colspan="5" class="py-6 text-center text-slate-400">Memuat riwayat...</td></tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div id="tabRewards" class="hidden overflow-x-auto">
                    <table class="w-full text-xs sm:text-sm">
                        ...
                        <tbody id="rewardsHistoryBody">
                            <tr><td colspan="4" class="py-6 text-center text-slate-400">Memuat riwayat...</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    <!-- ============ MODAL: CELEBRASI + KLAIM ============ -->
    <div id="claimModal" class="fixed inset-0 z-50 invisible transition-all duration-300 flex items-center justify-center p-4">
        <div id="claimBackdrop" onclick="closeClaimModal()" class="absolute inset-0 bg-slate-900/60 backdrop-blur-xs opacity-0 transition-opacity"></div>
        <div id="claimPanel" class="relative bg-white rounded-3xl max-w-sm w-full p-6 text-center shadow-2xl opacity-0 scale-95 transition-all duration-300 z-10 overflow-hidden">
            <div id="confettiContainer" class="absolute inset-0 pointer-events-none overflow-hidden"></div>

            <div class="relative z-10 space-y-4">
                <div class="w-16 h-16 bg-yellow-100 text-yellow-500 rounded-full flex items-center justify-center mx-auto text-3xl">
                ...
                <div id="claimAddressField" class="hidden text-left">
                    ...
                    <textarea id="claimAddress" rows="2" placeholder="Jl. Sudirman No. 123, Jakarta" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-emerald-500"></textarea>
                </div>

                <div id="claimResultBox" class="hidden bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-sm text-emerald-800 font-mono font-bold"></div>

                <button id="claimActionBtn" onclick="submitClaim()" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl shadow-lg transition-all">
                    Klaim Reward Sekarang 🎉
                </button>
                <button onclick="closeClaimModal()" class="w-full text-slate-400 text-xs font-semibold py-1">Tutup</button>
            </div>
        </div>
    </div>

    <!-- Toast -->
    <div id="toast" class="fixed bottom-5 right-5 z-50 hidden bg-slate-900 text-white px-4 py-3 rounded-xl shadow-xl flex items-center gap-3 border border-slate-700">
