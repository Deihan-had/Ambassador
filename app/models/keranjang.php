...
<body class="bg-slate-50 text-slate-800 flex flex-col min-h-screen">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16 gap-4">
                ...
                <div class="flex items-center gap-2 sm:gap-3">
                    <!-- Wishlist -->
                    <button onclick="<?php echo $isLoggedIn ? 'toggleWishlistModal()' : "window.location.href='views/auth/login.php'"; ?>"
                        ...
                        <?php if ($isLoggedIn) { ?>
                            <div class="relative" id="userMenuContainer">
                                <button onclick="toggleUserDropdown()"
                                    ...
                                    <div class="w-7 h-7 rounded-full bg-emerald-600 text-white font-bold flex items-center justify-center text-xs">
                                ...
                                <div id="userDropdown"
                                    ...
                                    <div class="px-4 py-2 border-b border-slate-100">
                                        <p class="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Akun Saya</p>
                                    ...
                                    <a href="logout.php"
                                        ...
                                        <i class="fa-solid fa-right-from-bracket text-red-500 w-4"></i><span>Keluar (Logout)</span>
                                    </a>
                                </div>
                            </div>
                        <?php } else { ?>
    ...
    <!-- Hero -->
    <section class="relative bg-gradient-to-r from-slate-900 via-slate-800 to-emerald-950 text-white overflow-hidden py-12 md:py-16">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div class="space-y-4 max-w-xl text-center md:text-left">
                <span class="inline-block bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-xs px-3 py-1 rounded-full font-semibold uppercase">
                    Mega Sale Edition
                </span>
                ...
                <p class="text-slate-300 text-sm md:text-base">
                    Dapatkan diskon hingga <span class="text-yellow-400 font-bold">50%</span> untuk produk pilihan dengan garansi resmi dan bebas biaya kirim.
                </p>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 w-full">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div class="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-none" id="categoryContainer"></div>
        ...
        <div id="emptyState" class="hidden text-center py-16">
            ...
            <p class="text-slate-500 text-sm max-w-sm mx-auto mb-4">Coba sesuaikan kata kunci pencarian atau pilih kategori lain.</p>
        ...
        <div id="cartPanel"
            ...
            <div class="p-4 border-t border-slate-100 bg-slate-50 space-y-3">
                ...
                <div class="space-y-1.5 text-sm pt-2 border-t border-slate-200">
                    <div class="flex justify-between text-slate-600"><span>Subtotal</span><span id="cartSubtotal">Rp 0</span></div>
                    <div class="flex justify-between text-slate-600"><span>Diskon</span><span id="cartDiscount" class="text-emerald-600 font-medium">- Rp 0</span></div>
                    <div class="flex justify-between text-slate-600"><span>Estimasi Ongkir</span><span id="cartShipping" class="text-slate-800 font-medium">Gratis</span></div>
    ...
    <!-- Detail Produk Modal -->
    <div id="productModal" class="fixed inset-0 z-50 invisible transition-all duration-300 flex items-center justify-center p-4">
        <div id="productModalBackdrop" onclick="closeProductModal()" class="absolute inset-0 bg-slate-900/60 backdrop-blur-xs opacity-0 transition-opacity"></div>
        <div id="productModalContent" class="relative bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl opacity-0 scale-95 transition-all duration-300 z-10">
            <button onclick="closeProductModal()" class="absolute top-4 right-4 bg-slate-100 hover:bg-slate-200 text-slate-600 w-9 h-9 rounded-full flex items-center justify-center z-20">
                <i class="fa-solid fa-xmark"></i>
            </button>
    ...
    <!-- Wishlist Modal -->
    <div id="wishlistModal" class="fixed inset-0 z-50 invisible transition-all duration-300 flex items-center justify-center p-4">
        <div id="wishlistBackdrop" onclick="toggleWishlistModal()" class="absolute inset-0 bg-slate-900/60 backdrop-blur-xs opacity-0 transition-opacity"></div>
        <div id="wishlistPanel" class="relative bg-white rounded-2xl max-w-2xl w-full max-h-[80vh] flex flex-col shadow-2xl opacity-0 scale-95 transition-all duration-300 z-10 overflow-hidden">
            <div class="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                <h3 class="font-bold text-slate-800 flex items-center gap-2"><i class="fa-solid fa-heart text-red-500"></i> Wishlist Saya</h3>
                <button onclick="toggleWishlistModal()" class="text-slate-400 hover:text-slate-600 p-2"><i class="fa-solid fa-xmark text-lg"></i></button>
            </div>
    ...
    <!-- Toast Notification -->
    <div id="toast" class="fixed bottom-5 right-5 z-50 hidden toast-slide-in bg-slate-900 text-white px-4 py-3 rounded-xl shadow-xl flex items-center gap-3 border border-slate-700">
    ...
    <footer class="bg-slate-900 text-slate-400 text-sm mt-16 border-t border-slate-800">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 grid grid-cols-1 md:grid-cols-4 gap-8">
            <div class="space-y-3">
                ...
                <p class="text-xs text-slate-400">Platform belanja online terpercaya dengan garansi kualitas terbaik.</p>
            </div>
        </div>
        <div class="border-t border-slate-800 py-4 text-center text-xs text-slate-500">
            © 2026 Ambassador Inc. Seluruh hak cipta dilindungi.
        </div>
    </footer>

    <!-- JS Variables Init & Helper -->
    <script>
